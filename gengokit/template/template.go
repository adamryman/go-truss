// Code generated by go-bindata.
// sources:
// NAME/NAME-cli-client/client_main.gotemplate
// NAME/NAME-server/server_main.gotemplate
// NAME/generated/cli/handlers/client_handler.gotemplate
// NAME/generated/client/grpc/client.gotemplate
// NAME/generated/client/http/client.gotemplate
// NAME/generated/endpoints.gotemplate
// NAME/generated/transport_grpc.gotemplate
// NAME/generated/transport_http.gotemplate
// NAME/handlers/server/hooks.gotemplate
// NAME/handlers/server/server_handler.gotemplate
// NAME/middlewares/endpoints.gotemplate
// NAME/middlewares/service.gotemplate
// DO NOT EDIT!

package template

import (
	"bytes"
	"compress/gzip"
	"fmt"
	"io"
	"io/ioutil"
	"os"
	"path/filepath"
	"strings"
	"time"
)

func bindataRead(data []byte, name string) ([]byte, error) {
	gz, err := gzip.NewReader(bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	var buf bytes.Buffer
	_, err = io.Copy(&buf, gz)
	clErr := gz.Close()

	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}
	if clErr != nil {
		return nil, err
	}

	return buf.Bytes(), nil
}

type asset struct {
	bytes []byte
	info  os.FileInfo
}

type bindataFileInfo struct {
	name    string
	size    int64
	mode    os.FileMode
	modTime time.Time
}

func (fi bindataFileInfo) Name() string {
	return fi.name
}
func (fi bindataFileInfo) Size() int64 {
	return fi.size
}
func (fi bindataFileInfo) Mode() os.FileMode {
	return fi.mode
}
func (fi bindataFileInfo) ModTime() time.Time {
	return fi.modTime
}
func (fi bindataFileInfo) IsDir() bool {
	return false
}
func (fi bindataFileInfo) Sys() interface{} {
	return nil
}

var _nameNameCliClientClient_mainGotemplate = []byte("")

func nameNameCliClientClient_mainGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameNameCliClientClient_mainGotemplate,
		"NAME/NAME-cli-client/client_main.gotemplate",
	)
}

func nameNameCliClientClient_mainGotemplate() (*asset, error) {
	bytes, err := nameNameCliClientClient_mainGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/NAME-cli-client/client_main.gotemplate", size: 3208, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameNameServerServer_mainGotemplate = []byte("")

func nameNameServerServer_mainGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameNameServerServer_mainGotemplate,
		"NAME/NAME-server/server_main.gotemplate",
	)
}

func nameNameServerServer_mainGotemplate() (*asset, error) {
	bytes, err := nameNameServerServer_mainGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/NAME-server/server_main.gotemplate", size: 3375, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameGeneratedCliHandlersClient_handlerGotemplate = []byte("")

func nameGeneratedCliHandlersClient_handlerGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameGeneratedCliHandlersClient_handlerGotemplate,
		"NAME/generated/cli/handlers/client_handler.gotemplate",
	)
}

func nameGeneratedCliHandlersClient_handlerGotemplate() (*asset, error) {
	bytes, err := nameGeneratedCliHandlersClient_handlerGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/generated/cli/handlers/client_handler.gotemplate", size: 744, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameGeneratedClientGrpcClientGotemplate = []byte("")

func nameGeneratedClientGrpcClientGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameGeneratedClientGrpcClientGotemplate,
		"NAME/generated/client/grpc/client.gotemplate",
	)
}

func nameGeneratedClientGrpcClientGotemplate() (*asset, error) {
	bytes, err := nameGeneratedClientGrpcClientGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/generated/client/grpc/client.gotemplate", size: 2200, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameGeneratedClientHttpClientGotemplate = []byte("")

func nameGeneratedClientHttpClientGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameGeneratedClientHttpClientGotemplate,
		"NAME/generated/client/http/client.gotemplate",
	)
}

func nameGeneratedClientHttpClientGotemplate() (*asset, error) {
	bytes, err := nameGeneratedClientHttpClientGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/generated/client/http/client.gotemplate", size: 105, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameGeneratedEndpointsGotemplate = []byte("")

func nameGeneratedEndpointsGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameGeneratedEndpointsGotemplate,
		"NAME/generated/endpoints.gotemplate",
	)
}

func nameGeneratedEndpointsGotemplate() (*asset, error) {
	bytes, err := nameGeneratedEndpointsGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/generated/endpoints.gotemplate", size: 2684, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameGeneratedTransport_grpcGotemplate = []byte("")

func nameGeneratedTransport_grpcGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameGeneratedTransport_grpcGotemplate,
		"NAME/generated/transport_grpc.gotemplate",
	)
}

func nameGeneratedTransport_grpcGotemplate() (*asset, error) {
	bytes, err := nameGeneratedTransport_grpcGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/generated/transport_grpc.gotemplate", size: 3655, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameGeneratedTransport_httpGotemplate = []byte("")

func nameGeneratedTransport_httpGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameGeneratedTransport_httpGotemplate,
		"NAME/generated/transport_http.gotemplate",
	)
}

func nameGeneratedTransport_httpGotemplate() (*asset, error) {
	bytes, err := nameGeneratedTransport_httpGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/generated/transport_http.gotemplate", size: 106, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameHandlersServerHooksGotemplate = []byte("")

func nameHandlersServerHooksGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameHandlersServerHooksGotemplate,
		"NAME/handlers/server/hooks.gotemplate",
	)
}

func nameHandlersServerHooksGotemplate() (*asset, error) {
	bytes, err := nameHandlersServerHooksGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/handlers/server/hooks.gotemplate", size: 62, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameHandlersServerServer_handlerGotemplate = []byte("")

func nameHandlersServerServer_handlerGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameHandlersServerServer_handlerGotemplate,
		"NAME/handlers/server/server_handler.gotemplate",
	)
}

func nameHandlersServerServer_handlerGotemplate() (*asset, error) {
	bytes, err := nameHandlersServerServer_handlerGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/handlers/server/server_handler.gotemplate", size: 62, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameMiddlewaresEndpointsGotemplate = []byte("")

func nameMiddlewaresEndpointsGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameMiddlewaresEndpointsGotemplate,
		"NAME/middlewares/endpoints.gotemplate",
	)
}

func nameMiddlewaresEndpointsGotemplate() (*asset, error) {
	bytes, err := nameMiddlewaresEndpointsGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/middlewares/endpoints.gotemplate", size: 76, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

var _nameMiddlewaresServiceGotemplate = []byte("")

func nameMiddlewaresServiceGotemplateBytes() ([]byte, error) {
	return bindataRead(
		_nameMiddlewaresServiceGotemplate,
		"NAME/middlewares/service.gotemplate",
	)
}

func nameMiddlewaresServiceGotemplate() (*asset, error) {
	bytes, err := nameMiddlewaresServiceGotemplateBytes()
	if err != nil {
		return nil, err
	}

	info := bindataFileInfo{name: "NAME/middlewares/service.gotemplate", size: 76, mode: os.FileMode(420), modTime: time.Unix(1464111000, 0)}
	a := &asset{bytes: bytes, info: info}
	return a, nil
}

// Asset loads and returns the asset for the given name.
// It returns an error if the asset could not be found or
// could not be loaded.
func Asset(name string) ([]byte, error) {
	cannonicalName := strings.Replace(name, "\\", "/", -1)
	if f, ok := _bindata[cannonicalName]; ok {
		a, err := f()
		if err != nil {
			return nil, fmt.Errorf("Asset %s can't read by error: %v", name, err)
		}
		return a.bytes, nil
	}
	return nil, fmt.Errorf("Asset %s not found", name)
}

// MustAsset is like Asset but panics when Asset would return an error.
// It simplifies safe initialization of global variables.
func MustAsset(name string) []byte {
	a, err := Asset(name)
	if err != nil {
		panic("asset: Asset(" + name + "): " + err.Error())
	}

	return a
}

// AssetInfo loads and returns the asset info for the given name.
// It returns an error if the asset could not be found or
// could not be loaded.
func AssetInfo(name string) (os.FileInfo, error) {
	cannonicalName := strings.Replace(name, "\\", "/", -1)
	if f, ok := _bindata[cannonicalName]; ok {
		a, err := f()
		if err != nil {
			return nil, fmt.Errorf("AssetInfo %s can't read by error: %v", name, err)
		}
		return a.info, nil
	}
	return nil, fmt.Errorf("AssetInfo %s not found", name)
}

// AssetNames returns the names of the assets.
func AssetNames() []string {
	names := make([]string, 0, len(_bindata))
	for name := range _bindata {
		names = append(names, name)
	}
	return names
}

// _bindata is a table, holding each asset generator, mapped to its name.
var _bindata = map[string]func() (*asset, error){
	"NAME/NAME-cli-client/client_main.gotemplate": nameNameCliClientClient_mainGotemplate,
	"NAME/NAME-server/server_main.gotemplate": nameNameServerServer_mainGotemplate,
	"NAME/generated/cli/handlers/client_handler.gotemplate": nameGeneratedCliHandlersClient_handlerGotemplate,
	"NAME/generated/client/grpc/client.gotemplate": nameGeneratedClientGrpcClientGotemplate,
	"NAME/generated/client/http/client.gotemplate": nameGeneratedClientHttpClientGotemplate,
	"NAME/generated/endpoints.gotemplate": nameGeneratedEndpointsGotemplate,
	"NAME/generated/transport_grpc.gotemplate": nameGeneratedTransport_grpcGotemplate,
	"NAME/generated/transport_http.gotemplate": nameGeneratedTransport_httpGotemplate,
	"NAME/handlers/server/hooks.gotemplate": nameHandlersServerHooksGotemplate,
	"NAME/handlers/server/server_handler.gotemplate": nameHandlersServerServer_handlerGotemplate,
	"NAME/middlewares/endpoints.gotemplate": nameMiddlewaresEndpointsGotemplate,
	"NAME/middlewares/service.gotemplate": nameMiddlewaresServiceGotemplate,
}

// AssetDir returns the file names below a certain
// directory embedded in the file by go-bindata.
// For example if you run go-bindata on data/... and data contains the
// following hierarchy:
//     data/
//       foo.txt
//       img/
//         a.png
//         b.png
// then AssetDir("data") would return []string{"foo.txt", "img"}
// AssetDir("data/img") would return []string{"a.png", "b.png"}
// AssetDir("foo.txt") and AssetDir("notexist") would return an error
// AssetDir("") will return []string{"data"}.
func AssetDir(name string) ([]string, error) {
	node := _bintree
	if len(name) != 0 {
		cannonicalName := strings.Replace(name, "\\", "/", -1)
		pathList := strings.Split(cannonicalName, "/")
		for _, p := range pathList {
			node = node.Children[p]
			if node == nil {
				return nil, fmt.Errorf("Asset %s not found", name)
			}
		}
	}
	if node.Func != nil {
		return nil, fmt.Errorf("Asset %s not found", name)
	}
	rv := make([]string, 0, len(node.Children))
	for childName := range node.Children {
		rv = append(rv, childName)
	}
	return rv, nil
}

type bintree struct {
	Func     func() (*asset, error)
	Children map[string]*bintree
}
var _bintree = &bintree{nil, map[string]*bintree{
	"NAME": &bintree{nil, map[string]*bintree{
		"NAME-cli-client": &bintree{nil, map[string]*bintree{
			"client_main.gotemplate": &bintree{nameNameCliClientClient_mainGotemplate, map[string]*bintree{}},
		}},
		"NAME-server": &bintree{nil, map[string]*bintree{
			"server_main.gotemplate": &bintree{nameNameServerServer_mainGotemplate, map[string]*bintree{}},
		}},
		"generated": &bintree{nil, map[string]*bintree{
			"cli": &bintree{nil, map[string]*bintree{
				"handlers": &bintree{nil, map[string]*bintree{
					"client_handler.gotemplate": &bintree{nameGeneratedCliHandlersClient_handlerGotemplate, map[string]*bintree{}},
				}},
			}},
			"client": &bintree{nil, map[string]*bintree{
				"grpc": &bintree{nil, map[string]*bintree{
					"client.gotemplate": &bintree{nameGeneratedClientGrpcClientGotemplate, map[string]*bintree{}},
				}},
				"http": &bintree{nil, map[string]*bintree{
					"client.gotemplate": &bintree{nameGeneratedClientHttpClientGotemplate, map[string]*bintree{}},
				}},
			}},
			"endpoints.gotemplate": &bintree{nameGeneratedEndpointsGotemplate, map[string]*bintree{}},
			"transport_grpc.gotemplate": &bintree{nameGeneratedTransport_grpcGotemplate, map[string]*bintree{}},
			"transport_http.gotemplate": &bintree{nameGeneratedTransport_httpGotemplate, map[string]*bintree{}},
		}},
		"handlers": &bintree{nil, map[string]*bintree{
			"server": &bintree{nil, map[string]*bintree{
				"hooks.gotemplate": &bintree{nameHandlersServerHooksGotemplate, map[string]*bintree{}},
				"server_handler.gotemplate": &bintree{nameHandlersServerServer_handlerGotemplate, map[string]*bintree{}},
			}},
		}},
		"middlewares": &bintree{nil, map[string]*bintree{
			"endpoints.gotemplate": &bintree{nameMiddlewaresEndpointsGotemplate, map[string]*bintree{}},
			"service.gotemplate": &bintree{nameMiddlewaresServiceGotemplate, map[string]*bintree{}},
		}},
	}},
}}

// RestoreAsset restores an asset under the given directory
func RestoreAsset(dir, name string) error {
	data, err := Asset(name)
	if err != nil {
		return err
	}
	info, err := AssetInfo(name)
	if err != nil {
		return err
	}
	err = os.MkdirAll(_filePath(dir, filepath.Dir(name)), os.FileMode(0755))
	if err != nil {
		return err
	}
	err = ioutil.WriteFile(_filePath(dir, name), data, info.Mode())
	if err != nil {
		return err
	}
	err = os.Chtimes(_filePath(dir, name), info.ModTime(), info.ModTime())
	if err != nil {
		return err
	}
	return nil
}

// RestoreAssets restores an asset under the given directory recursively
func RestoreAssets(dir, name string) error {
	children, err := AssetDir(name)
	// File
	if err != nil {
		return RestoreAsset(dir, name)
	}
	// Dir
	for _, child := range children {
		err = RestoreAssets(dir, filepath.Join(name, child))
		if err != nil {
			return err
		}
	}
	return nil
}

func _filePath(dir, name string) string {
	cannonicalName := strings.Replace(name, "\\", "/", -1)
	return filepath.Join(append([]string{dir}, strings.Split(cannonicalName, "/")...)...)
}

